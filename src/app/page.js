...
export default function Home() {
  ...
  const fetchLists = useCallback(async () => {
    ...
    try {
      ...
      else addToast(cJson.error || "Error loading contributions", "error");
      if (rRes.ok) setReceipts(rJson || []);
      else addToast(rJson.error || "Error loading receipts", "error");
      if (eRes.ok) setExpenses(eJson || []);
      else addToast(eJson.error || "Error loading expenses", "error");
    } catch (err) {
  ...
  const submitContribution = async (e) => {
    ...
    try {
      const res = await fetch(`${apiBase}/api/contributions`, {
        method: "POST",
        headers: authHeaders(),
        body: JSON.stringify({ gbp_amount: Number(contribAmount), note: contribNote }) // GBP now
      });
      const json = await res.json();
      if (res.ok) {
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            {(role === "developer" || role === "admin") && (
              <a className="btn-primary mt-3 inline-flex" href="/daily-journal">
                Go to Daily Cost Journal
              </a>
            )}
          </div>
