...
export default function Home() {
  ...
  const fetchContribs = useCallback(async () => {
    ...
    try {
      const params = new URLSearchParams({
        ...
        ...(cFilters.startDate ? { startDate: cFilters.startDate } : {}),
        ...(cFilters.endDate ? { endDate: cFilters.endDate } : {}),
        ...(cFilters.status ? { status: cFilters.status } : {}),
        ...(cFilters.investor_id ? { investor_id: cFilters.investor_id } : {})
      }).toString();
  ...
  }, [session, cFilters, cPage, addToast]);
  ...
  const fetchExpenses = useCallback(async () => {
    ...
    try {
      const params = new URLSearchParams({
        page: String(ePage),
        limit: String(PAGE_SIZE),
        ...(eFilters.startDate ? { startDate: eFilters.startDate } : {}),
        ...(eFilters.endDate ? { endDate: eFilters.endDate } : {}),
        ...(eFilters.status ? { status: eFilters.status } : {}),
        ...(eFilters.category ? { category: eFilters.category } : {})
      }).toString();
  ...
  }, [session, eFilters, ePage, addToast]);
  ...
  const approvedContribs = useMemo(
    () => visibleContribs.filter((c) => c.status === "approved"),
    [visibleContribs]
  );
